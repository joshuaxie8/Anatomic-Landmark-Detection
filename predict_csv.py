...
def main():
    ...
    # Save predictions using csv module
    save_predictions_csv(predictions, config.dataRoot + config.predictionsPath)
...
def save_predictions_csv(predictions_df, output_path):
    """Save predictions using Python's built-in csv module"""
    print(f"Saving predictions to: {output_path}")
    
    try:
        with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
            # Get column names
            fieldnames = list(predictions_df.columns)
        ...
        print("Predictions saved successfully using csv module!")
        ...
        if os.path.exists(output_path):
            ...
            # Try to read first few lines to verify
