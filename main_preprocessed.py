...
from dataLoader_preprocessed import ToTensor, LandmarksDataset
import models
import train
import lossFunction
import argparse
...
parser.add_argument("--batchSize", type=int, default=4)  # Increased default batch size
...
parser.add_argument("--supervised_dataset_train", type=str, default="TrainingData_preprocessed/")
parser.add_argument("--supervised_dataset_test", type=str, default="Test2Data_preprocessed/")
...
parser.add_argument("--numWorkers", type=int, default=16)  # Increased default workers
...
def main():
	...
	# Use only ToTensor transform for preprocessed images (no rescaling needed)
	transform_preprocessed = torchvision.transforms.Compose([
					ToTensor()
					])
	...
	train_dataset_origin = LandmarksDataset(csv_file=config.dataRoot + config.trainingSetCsv,
												...
												transform=transform_preprocessed,
