...
def get_transforms_for_preprocessed_images():
    """
    Returns transforms optimized for preprocessed images (no rescaling needed).
    Images are already the correct size, so we only need ToTensor.
    """
    return [ToTensor()]


def get_transforms_for_original_images(image_scale):
    """
    Returns transforms for original images that need rescaling.
    """
    return [
        Rescale(image_scale),
        ToTensor()
